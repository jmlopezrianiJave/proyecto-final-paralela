...
vector<vector<Cell>> move_rabbits(
    ...
    for (int i = 0; i < R; ++i) {
        for (int j = 0; j < C; ++j) {
            ...
            if (can_procreate) place_rabbit(world_next, ox, oy, 0);
        }
    }
...
vector<vector<Cell>> move_foxes(
    ...
    for (int i = 0; i < R; ++i) {
        for (int j = 0; j < C; ++j) {
            ...
            if (can_procreate) place_fox(world_next, ox, oy, 0, 0);
        }
    }
...
int main() {
    // 1) Leer TODO del archivo redirigido por stdin
